      ...
      {!showModal ? (
        <>
          <div className="rss-page-header">
            <h1 className="rss-page-title">USERS</h1>
            <nav className="rss-breadcrumb">
              <span className="rss-bc-item">Team</span>
              <span className="rss-bc-sep">›</span>
              <span className="rss-bc-active">Users</span>
            </nav>
          </div>

          <div className="rss-actions-final">
            <button className="rss-btn-delete-final" onClick={() => alert('Select users to delete')}>
              <span className="rss-btn-icon-v2">🗑️</span> Delete User
            </button>
            <button className="rss-btn-add-final" onClick={handleAdd}>
              <span className="rss-btn-icon-v2">⊕</span> Add Member
            </button>
          </div>
            ...
            <table className="rss-table">
              <thead>
                <tr>
                  ...
                  <th className="rss-th-cat">👤 Name</th>
                  <th className="rss-th-sub">🎭 Role</th>
                  <th className="rss-th-status">⚡ Status</th>
                  <th className="rss-th-action">⚙️ Actions</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  paginatedMembers.map((member, index) => (
                    <tr key={member._id} className="rss-row">
                      <td className="rss-cell-num">{startIdx + index + 1}</td>
                      <td className="rss-cell-cat">
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          {member.imageUrl ? (
                            <img src={`${API_BASE}${member.imageUrl}`} alt={member.name} style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover' }} />
                          ) : (
                            <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#e8380d', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
                      ...
                      <td className="rss-cell-sub">{member.role}</td>
                      <td className="rss-cell-status">
                        <div className={`rss-toggle ${member.status ? 'active' : ''}`} onClick={() => handleToggleStatus(member._id, member.status)}>
                          <div className="rss-toggle-label">{member.status ? 'On' : 'Off'}</div>
                          <div className="rss-toggle-handle"></div>
                        </div>
                      </td>
                      <td className="rss-cell-action">
                        <div className="rss-action-btns">
                          <button className="rss-edit-btn-new" title="Edit" onClick={() => handleEdit(member)}>
                            📝
                          </button>
                          <button className="rss-delete-btn-new" title="Delete" onClick={() => handleDelete(member._id)}>
                            🗑️
                          </button>
                        </div>
                      </td>
