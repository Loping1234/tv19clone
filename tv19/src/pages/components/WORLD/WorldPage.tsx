...
export default function WorldPage() {
    ...
    const fetchRegionNews = useCallback(async (region: string, isLoadMore: boolean = false) => {
        ...
        try {
            const query = region === 'All Stories' ? 'world' : region;
            const currentSkip = isLoadMore ? skipRef.current + 30 : 0;
            const response = await getWorld(query, undefined, 30, false, currentSkip);
            ...
            console.error('world region fetch failed:', err);
            if (!isLoadMore) {
        ...
        <main className="world-region-page">
            <section className="world-region-shell">
                ...
                <div className="world-content-grid">
                    <section className="world-main-column" aria-live="polite">
                        {loading ? (
                            <div className="world-loader">
                                <div className="world-loader__spinner" />
                                <p>Loading {activeRegion} stories...</p>
                            </div>
                        ) : error ? (
                            <div className="world-empty">{error}</div>
                        ) : articles.length === 0 ? (
                            ...
                            <>
                                {fallbackNotice && (
                                    <div className="world-fallback-note">{fallbackNotice}</div>
                                )}

                                {heroArticle && (
                                    <a href={heroArticle.url} target="_blank" rel="noopener noreferrer" className="world-hero-card">
                                        <div className="world-hero-card__image-wrap" style={!heroArticle.image || failedImages.has(heroArticle.image) ? { background: 'linear-gradient(135deg, #e8e8e8, #f0f0f0)' } : undefined}>
                                            {heroArticle.image && !failedImages.has(heroArticle.image) ? (
                                                <img src={heroArticle.image} alt={heroArticle.title} className="world-hero-card__image" onError={() => handleImageError(heroArticle)} />
                                            ) : (
                                                <div className="world-hero-card__placeholder" />
                                            )}

                                            <div className="world-hero-card__overlay">
                                                <h1 className="world-hero-card__title">{heroArticle.title}</h1>
                                                {heroArticle.description && <p className="world-hero-card__desc">{heroArticle.description}</p>}
                                                <div className="world-story-item__meta">
                                ...
                                <div className="world-story-list">
                                    {mainListArticles.map((article, index) => (
                                        <a key={`${article.title}-${index}`} href={article.url} target="_blank" rel="noopener noreferrer" className="world-story-item">
                                            <div className="world-story-item__thumb-wrap" style={!article.image || failedImages.has(article.image) ? { background: 'linear-gradient(135deg, #e8e8e8, #f0f0f0)' } : undefined}>
                                                {article.image && !failedImages.has(article.image) ? (
                                                    <img src={article.image} alt={article.title} className="world-story-item__thumb" onError={() => handleImageError(article)} />
                                                ) : (
                                                    <div className="world-story-item__thumb-placeholder" />
                                                )}
                                            </div>
                                            <div className="world-story-item__content">
                                                <h2 className="world-story-item__title">{article.title}</h2>
                                                {article.description && <p className="world-story-item__desc">{article.description}</p>}
                                                <div className="world-story-item__meta">
                                ...
                                <div ref={observerTarget} className="infinite-scroll-loading" style={{ margin: '30px 0', textAlign: 'center' }}>
                                    {loadingMore && (
                                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                                            <div className="world-loader__spinner" style={{ width: '30px', height: '30px', borderWidth: '3px' }} />
                    ...
                    {articles.length > 0 && (
                        <aside className="world-sidebar">
                            <section className="world-sidebar-section">
                                <h3 className="world-sidebar-section__header">Trending in world</h3>
                            ...
                            <section className="world-sidebar-section">
                                <h3 className="world-sidebar-section__header">Related Sections</h3>
                                <div className="world-story-item__meta">Coming soon...</div>
                            </section>
                        </aside>
                    )}
                </div>
