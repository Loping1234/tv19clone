...
import { getPolitics, scrapeFallbackImage, type Article } from '../../../services/newsService';
    ...
    return (
        <main className="politics-region-page">
            <section className="politics-region-shell">
                <div className="politics-subnav-wrapper">
                    <button className="politics-nav-btn prev" aria-label="Scroll left" onClick={() => scroll('left')}><UilAngleLeft /></button>
                    <div className="politics-subnav" role="tablist" aria-label="Politics regions" ref={scrollRef}>
                        {REGIONS.map((region) => (
                            <button
                                key={region}
                                type="button"
                                ...
                                className={`politics-subnav__item ${activeRegion === region ? 'active' : ''}`}
                    ...
                    <button className="politics-nav-btn next" aria-label="Scroll right" onClick={() => scroll('right')}><UilAngleRight /></button>
                </div>
                                    ...
                                    <a href={heroArticle.url} target="_blank" rel="noopener noreferrer" className="politics-hero-card">
                                        <div className="politics-hero-card__image-wrap" style={!heroArticle.image || failedImages.has(heroArticle.image) ? { background: 'linear-gradient(135deg, #e8e8e8, #f0f0f0)' } : undefined}>
                                            {heroArticle.image && !failedImages.has(heroArticle.image) ? (
                                                ...
                                                <div className="politics-hero-card__placeholder" />
                                            )}
                                        ...
                                        <a key={`${article.title}-${index}`} href={article.url} target="_blank" rel="noopener noreferrer" className="politics-story-item">
                                            <div className="politics-story-item__thumb-wrap" style={!article.image || failedImages.has(article.image) ? { background: 'linear-gradient(135deg, #e8e8e8, #f0f0f0)' } : undefined}>
                                                {article.image && !failedImages.has(article.image) ? (
                                                    ...
                                                    <div className="politics-story-item__thumb-placeholder" />
                                                )}
                                            </div>
